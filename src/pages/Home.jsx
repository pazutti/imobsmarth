    ...
    <main>
      <header className="sticky">
        <div className="container" style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <button className="btn btn-primary" onClick={onLogin}>Entrar agora</button>
        </div>
      </header>
      ...
      <section className="container" style={{display:'grid', gap:24, gridTemplateColumns:'1.1fr 0.9fr', alignItems:'center'}}>
        <div>
          ...
          <p style={{ marginTop: 12, fontSize: 18, color: '#475569' }}>Gere laudos com fotos por cômodo e feche contratos em minutos. Reduza custos sem abrir mão da segurança.</p>
          <div style={{ marginTop: 16, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <button className="btn btn-primary" onClick={onLogin}>Começar grátis</button>
          </div>
        ...
        <div className="card" style={{background:'linear-gradient(135deg,#4f46e5,#7c3aed)', color:'#fff'}}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:12, fontSize:13 }}>
            <div style={{ background:'rgba(255,255,255,.1)', padding:10, borderRadius:12 }}>
              <div style={{opacity:.9}}>Convencional</div>
              <div style={{fontWeight:800}}>{formatCurrency(convencional)}</div>
            </div>
            <div style={{ background:'rgba(255,255,255,.1)', padding:10, borderRadius:12 }}>
              <div style={{opacity:.9}}>ImobIA</div>
              <div style={{fontWeight:800}}>{formatCurrency(imobia)}</div>
            </div>
            <div style={{ background:'rgba(16,185,129,.25)', padding:10, borderRadius:12 }}>
              <div style={{opacity:.9}}>Economia</div>
              <div style={{fontWeight:800}}>{formatCurrency(economia)}</div>
            </div>
          </div>
          <div style={{ marginTop: 10, fontSize: 12, opacity:.9 }}>Estimativas: vistoria {formatCurrency(vistConv)}, contrato {formatCurrency(contConv)}. ImobIA a partir de {formatCurrency(vistIA)} por item.</div>
        </div>
      </section>
    </main>
  )
